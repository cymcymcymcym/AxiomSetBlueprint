...
\end{theorem}
\begin{proof}
    Given any such $a$ and $b$, by $\Cref{add-inverses}$ there exists $(a)$ such that $a + (-a) = 0$. Then, we have $(-a) + (a+b) = (-a) + a$. \Cref{assoc-add} gives $( (-a) + a) + b = (-a) + a$. Then, \Cref{commu-add} gives $(-a) + a = a + (-a) = 0$, and substituting gives $0 + b = 0$. Then, $0 + b = b+0 = b$ by \Cref{commu-add} and \Cref{add-identity}, so we have $b=0$ by substitution, completing the proof. 
\end{proof}
...
\end{theorem}
\begin{proof}
    Given $a, b, b' \in R$ such that $a+b = a+b'$, there exists $(-a) \in R$ such that $a + (-a) = 0$ by \Cref{add-inverses}. Then, we have $(-a) + a = a + (-a) = 0$ by \Cref{commu-add}. Then, we have $(-a) + (a+b) = (-a) + (a + b')$, and so we have $(-a) + (a+b) = ((-a)+a)+b = 0 + b$ by \Cref{assoc-add} and substitution. Then, $0+b = b+0 = b$ by \Cref{commu-add} and $\Cref{add-identity}$. Substituting again gives $(-a) + (a+b) = b$. The same process shows $(a-) + (a+b') = b'$. Substituting once more gives $b = b'$, as desired.  
\end{proof}

\begin{theorem}[Additive Inverses are Unique]\label{add-inverse-unique}
    Given $a, x$ such that $a+ x = 0$, $x$ must be equal to $(-a)$. 
\end{theorem}
\begin{proof} If such $a$ and $x$ exist, then there exists $(-a)$ such that $a + (-a) = 0$. Then, we have $(-a) + (a+ x) = (-a) + 0$. Then, by \Cref{assoc-add}, we have $(-a) + (a+x) = ((-a)+a) + x = 0+x = x+0 = x$ and the fact that $(-a)+a = a + (-a) = 0$ by \Cref{commu-add} and \Cref{add-identity}. Also notice $(-a) + 0 = (-a)$. Substituting gives $x = (-a)$, as desired.
\end{proof}
...
\end{theorem}
\begin{proof}
    We have $a + (-a) = 0$ by \Cref{add-inverses}. But \Cref{commu-add} gives $(-a) + a = 0$, so $a$ is a solution to $(-a) + x = 0$. But, the solution to this is uniquely $-(-(a))$ by \Cref{add-inverse-unique}, and so we have $a = -(-a))$. 
\end{proof}
...
\end{theorem}
\begin{proof}
We have $0 + 0 = 0$ by \Cref{add-inverses}, so $0=0+0$ by \Cref{commu-add}. Then, we have $a \cdot (0) = a \cdot (0+0)$, and then $a \cdot 0 = a\cdot 0 + a\cdot 0$ by distributivity. Then, this becomes $a \cdot 0 + a \cdot 0 = a\cdot 0$, and so we have by $\Cref{add-inverse-unique}$ $a \cdot 0= 0$. 
\end{proof}
...
\end{theorem}
\begin{proof}
There exists $(-(ab))$ such that $ab + (-(ab)) = 0$. Then, there exists $(-b)$ such that $b + (-b) = 0$. Also note $ab + a(-b)) = a(b+(-b)) = a(0) = 0$ by \Cref{dist-mult-over-add}, substitution, and \Cref{mult-by-0-is-0}. Then, by \Cref{add-inverse-unique}, we have $-(ab) = a(-b)$.
\end{proof}
...
\end{theorem}
\begin{proof}
We have $-((-a)b) = (-a)(-b)$ by \Cref{right-dist-inverses} and $-((-a)b) = ((-(-a))b) = ab$ by \Cref{left-dist-inverses} and \Cref{inverse-of-inverse-is-original}. So, we have $ab = (-a)(-b)$. 
\end{proof}
...
\end{theorem}
\begin{proof}
$a \cdot 1 = a$ by \Cref{mult-identity}, $-(a \cdot 1) = a \cdot (-1)$ by \Cref{right-dist-inverses}, and substitution gives $-(a) = a \cdot (-1)$. Then, we have $-a = (-1)a$ by \Cref{commu-mult}.
\end{proof}
...
\end{theorem}
\begin{proof}
First, we show $a+(-b)$ is a solution. Notice that $b + (a+(-b)) = b + ((-b) + a) = (b + (-b)) + a = 0 + a = a + 0 = a$ via application of \Cref{assoc-add}, \Cref{commu-add} and \Cref{add-inverses}. \\

Then, for any given solution to the equation $y$ to the equation $b+x = a$, we show $x = a+(-b)$, showing uniqueness. We have $(-b) + b+x = (-b) + a$, and $(-b) + b + x = ((-b)+b) + x = (b+(-b)) + x = 0 + x$ by \Cref{assoc-add}, \Cref{commu-add}, and \Cref{add-identity}, and similarly $(-b) + a = a + (-b)$ by \Cref{commu-add}, so we have $x = a + (-b)$ for any solution to this equation.
\end{proof}
...
\end{lemma}
\begin{proof}
    Since $0+0 = 0$ by \Cref{add-identity}, we have $(-0) = 0$ by \Cref{add-inverse-unique}.
\end{proof}
...
\end{theorem}
\begin{proof}
    We have $ab = a$, and $a(b-1) = ab - a = a - a = 0$. Then, since $a, b$ are in an integral domain, $a(b-1) = 0$ implies $a = 0$ or $b-1 = 0$ by \Cref{def-int-dom}. But, $a \neq 0$, so we have $b-1 = 0$ and therefore $b = 1$. 
\end{proof}
...
\end{lemma}
\begin{proof}
    We have $0 = 0$, so $0 \notin O^+$ as $0 =0$ and by \Cref{trichotomy} exactly one of $0 \in P$, $0 =0$, and $(-0) \in P$ is true.
\end{proof}
...
\end{theorem}
\begin{proof}
    Assume $(-1) \in P$. Then, we have $(-1) \cdot (-1) = 1\in P$ by \Cref{pos-mult-closure}, a contradiction of trichotomy. Therefore, $(-1) \notin P$. So, we must have either $1 \in P$ or $1=0$. The latter is not possible, so we have $1 \in P$.  
\end{proof}
...
\end{theorem}
\begin{proof}
We have $a<b$ or $a=b$ and $a<c$ or $a \le c$. Now, there are four cases. Firstly, $a<b$ and $b < c$. Then, $b-a \in P$ and $c-b \in P$ by \Cref{def-le-and-leq}, so adding these gives $c-b + b-a = c-a \in P$ by \Cref{pos-mult-closure}. Therefore, $-(c-a) = a-c \notin P$ by \Cref{trichotomy}, so $c \nless a$ and therefore $a \le c$. All other cases follow in the same manner (but utilizing direct substitution of equals instead of positive multiplicative closure), so we neglect to include them.
\end{proof}
...
Case 1: $a < b$ and $x < y$. We have $b-a \in P$ and $y-x \in P$, so $b-a + (y-x) = (b+y)-(a+x) \in P$ by \Cref{pos-add-closure}, so we have $-((b+y)-(a+x)) = (a+x) - (b+y) \notin P$ and therefore $b+y \nless a+x$ and so $a+x \le b+y$. \\
...
\end{theorem}
\begin{proof}
We have $a < b$ or $a = b$ and $x < y$ or $x=y$ by \Cref{leq-le-or-eq}. So, we have four cases. \\

Case 1: $a<b$ and $x<y$. Then, we have $b-a \in P$, $y-x \in P$. So, we have $x(b-a) = bx-xa \in P$ by \Cref{pos-mult-closure}, and similarly $b(y-x) = yb - bx \in P$ by \Cref{pos-mult-closure}. So, we get $yb-bx + bx - xa = by - ax \in P$ by \Cref{pos-mult-closure}, and so $-(by-ax) = ax-by \notin P$ by \Cref{trichotomy}, and therefore $by \nless ax$ and so $ax \le by$. \\

Case 2: $a < b$, $x=y$. We have $b-a \in P$, so $(b-a)x = bx - ax = by - ax \in P$ by \Cref{pos-mult-closure}, and so $-(by-ax) = ax-by \notin P$ by \Cref{trichotomy}, and therefore $by \nless ax$ and therefore $ax \le by$. \\
...
\begin{proof}
    ...
    By \Cref{trichotomy}, if $a,b \neq 0$, then either $a \in P$ or $(-a) \in P$ and $b \in P$ or $(-b) \in P$. We have $4$ cases: \\
    
    Case 1: $a \in P, b \in P$. Then, $ab \in P$ by \Cref{pos-mult-closure}, and so by \Cref{trichotomy}, $ab \neq 0$. \\
    
    Case 2: $a \in P, b \notin P$. Then, $b \neq 0$, so $-b \in P$ by \Cref{trichotomy}. Therefore, we have $a(-b) = -(ab) \in P$ by \Cref{pos-mult-closure}, and so $ab \neq 0$ by \Cref{trichotomy}. \\
    
    Case 3: $a \notin P, b \in P$. By symmetry with case $2$ (the order is merely flipped), we get that $ab \neq 0$ again. \\
    
    Case 4: $a \notin P, b \notin P$. then, $a, b \neq 0$, so $(-a), (-b) \in P$ by \Cref{pos-mult-closure}. Then, $(-a)(-b) = ab \in P$, so $ab \neq 0$ by \Cref{trichotomy}. \\
...
\begin{proof}
    This follows from \Cref{mult-cancel-in-int-dom} and \Cref{local-identity-one-in-int-dom}. 
\end{proof}
...
\end{lemma}
\begin{proof}
    By \Cref{mult-identity}, any integer $a>1$ has $a=a\cdot 1$, so $a$ and $1$ are two distinct divisors of $a$ because $a>1$.
\end{proof}
...
\begin{proof}
    By \Cref{atleast2divisors}, a composite number has at least two divisors. A composite number does not have exactly two divisors because it is not prime. Thus, a composite number $a$ has more than two divisors, indicating there exist $x,y\in\ZZ^+$ and $x,y\neq 1,a$ such that $a=xy$.
\end{proof}
...
\begin{proof}
    ...
    by \Cref{mult-identity} and \Cref{mult-by-0-is-0}. Therefore, $n_0 \cdot n_0$ is smaller than $n_0$ and in $S$, so $n_0$ is not the least element. This is a contradiction, so $S$ must be empty. Hence, there does not exist an integer between 1 and 0.
\end{proof}
...
\begin{proof}
    ...
    \[b=a \cdot t\]
    by \Cref{mult-identity}, this implies
    \[1 \cdot b=a \cdot t\]
    If $a>b$ and $t>1$ 
    \[a \cdot t > b \cdot 1\]
    but by \Cref{pos-mult-compatible-leq}, this is not possible, so $t<1$, since $t=1$ implies $a=b$, which contradicts our assumption. Moreover, since $a,b \in \ZZ^+$, $t \in \ZZ^+$ by \Cref{pos-mult-closure} and \Cref{trichotomy}. This implies that
    \[0<t<1\]
    which is not possible by \Cref{nibzo} since $t \in \ZZ$. Therefore, since $a$ cannot be greater than $b$, by \Cref{pos-mult-compatible-leq}, $a \leq b$.
\end{proof}
...
\begin{proof}
    ...
    We know $k_0\neq n+1$ by \Cref{def_product_notation}, $\prod_{i=n}^{n+1} a_i=\left(\prod_{i=n}^n a_i\right) a_{n+1}=a_n \left(\prod_{i=n+1}^{n+1} a_i\right) $. Thus, $k_0-1>n$ and $k_0-1 \not\in S$ implies
    \begin{equation*}
...
\begin{proof}
\noindent Since for all $x \in S$, $x\leq \min{(a,b)} $  by \Cref{a_div_b_then_a_leq_b}, we know $S$ is bounded above. Thus, by (\Cref{gen_wop}), there exists a maximum element $d$ in $S$, which by definition makes $d$ the GCD of $a$ and $b$. This shows existence of a GCD for any two positive integers. \\

Now, we show the uniqueness of $d$. Suppose there exists $d' \in S$ such that $d'\neq d$, $d'$ is also the GCD of $a$ and $b$. Then, by definition of GCD (\Cref{def_GCD}), $d' \geq d$ and $d\geq d'$. Thus, $d'=d$, a contradiction. Therefore, no such $d'$ can exist, showing uniqueness.
\end{proof}
...
\begin{proof}
    Given that $a$ and $b$ are not zero, without loss of generality assume $a$ is the nonzero one. Then, if $a > 0$, we have $a \cdot 1 + b \cdot 0 = a \in \ZZ^+$, so $a \cdot 1 + b \cdot 0 \in S$. Otherwise, $a <0$, so $a \cdot (-1) + b \cdot 0 = (-a)$. Then, $(-a) \in \ZZ^+$, so $(-a) \ge 1 \ge 0$, and so $(-a) \in \ZZ^+$ and $a \cdot (-1) + b \cdot 0 \in S$. In both cases, $S$ is nonempty. Therefore, such a least element $d$ exists. 
     ...
     Now, we must show that $d \mid a$, $d \mid b$, then show $d = \text{gcd} (a,b)$. Since $d$ is in $S$, we can write $d = an + bm$, where $an$, $bn$ are in $\ZZ$. Then, since $d>0$, by \Cref{division_alg}, $a=dq+r$ for some $q_1, r_1 \in \ZZ$ with $0 \leq r_1 < d$. \\
    ...
    Now consider the $e$ in $\ZZ^+$ such that $e \mid a$, $e \mid b$. Assume there is some $e$ such that $e > d$. By \Cref{linearity}, $e \mid (an+bm)$, so $e \mid d$. But by \Cref{a_div_b_then_a_leq_b}, $e \mid d$ implies that $e \leq d$. This is a contradiction, so for all common divisors of $a$ and $b$, $e\le d$. Therefore, by \Cref{def_GCD}, $d$ is the GCD of $a$ and $b$. 
\end{proof}
...
\end{lemma}
\begin{proof} First, we show the if direction. If $p=q$, then $p=q\cdot 1$ by \Cref{mult-identity}, so $p\mid q$ by the definition of divisor (\Cref{divisor}). Now, the only if direction. If $p\mid q$, $p$ is a divisor of $q$. The only two divisors of $q$ are $1$ and $q$ by definition of prime (\Cref{def_prime}). $p\neq 1$ because $1$ is not prime, so $p=q$.
\end{proof}
...
\end{lemma}
\begin{proof}
    Recall the definition of GCD (\Cref{def_GCD}): the maximum element of the set $S = \{ x \mid x \mid a \text{ and } x \mid p ,\  x \in \ZZ^+ \}$ is $\gcd(a,p)$. Suppose $D_p$ denote the set of divisor of $p$, then $S\subseteq D_p$ because every element in $x\in S$ has $x\mid p$ which implies $x\mid p$. By the definition of prime (\Cref{def_prime}), $S\subseteq D_p=\left\{1,p\right\}$, so $\gcd(a,p)=1 \text{ or } p$. We know $p\nmid a$, so $p$ is the the GCD, which indicates $\gcd(a,p)=1$.
\end{proof}
...
\end{theorem}
\begin{proof}
    $a\mid bc$ means $bc=aq$ for some $q\in\ZZ$ by definition of divisor (\Cref{divisor}). By Bezout's identity (\Cref{Bezout}), $(a,b)=1$ implies there exists $x,y\in\ZZ$ such that $ax+by=1$. Multiplying both sides by $c$, 
    \begin{equation*}
        \begin{aligned}
            cax+cby&=c\\
            a(cx)+(aq)y&=c\\
        ...
        \end{aligned}
    \end{equation*}
    which implies $a\mid c$ by definition of divisor (\Cref{divisor}).
\end{proof}
...
\end{theorem}
\begin{proof}
    If $p\mid a$, we're done. If $p\nmid a$, then $(a,p)=1$ by \Cref{prime_coprime_with_nondiv_int}. Further, by \Cref{coprime_imply_division}, $p\mid ab$ and $(p,a)=1$ implies $p\mid b$. 
\end{proof}
...
\begin{proof}
    ...
    If $k_0=1$, then $p\mid\prod_{i=1}^{k_0} a_i=\prod_{i=1}^1=a_1$ by definition of product notation (\Cref{def_product_notation}), so $p\mid a_1$, and we're done. 

    If $k_0\neq 1$, then $k_0-1\geq1$ and $k_0-1\notin S$ implies $p\nmid \prod_{i=1}^{k_0-1}$. By definition of product notation (\Cref{def_product_notation}), $\prod_{i=1}^{k_0} a_i=(\prod_{i=1}^{k_0-1} a_i) a_{k_0}$. Then, $p\mid \prod_{i=1}^{k_0} a_i=(\prod_{i=1}^{k_0-1} a_i) a_{k_0}$ and $p\nmid \prod_{i=1}^{k_0-1}$ implies $p\mid a_{k_0}$ by \Cref{euclid-lemma}.
\end{proof}
...
We have $t = \prod_{i=1}^{n} p_i = \prod_{i=1}^m q_i$ by assumption. Then, consider the first integer $k$ such that $p_k \neq q_k$ and let $j = k-1$. Such a $k$ exists since there is at least one such $i$ by assumption, and therefore by well-ordering there is a least such $i$. Then, we have $p_j = q_j$, and Applying \Cref{prod_split_from_middle} and \Cref{def_product_notation}, we have:
...
Now, since every prime is nonzero by definition (\Cref{def_prime}), we write $t = p_j y$ for some $y \in \ZZ^+$ and see:
...
by \Cref{prod_split_from_middle} and \Cref{prod_re_index}, and similarly we see:
...
where $\{a_i\}$ and $\{b_i\}$ are ordered prime factorizations and $a_j \neq b_j$ for some $j \in \ZZ^+$. Therefore, $y \in \ZZ^+$ and $y \in T$. But, $y | t$, so $y < t$ by \Cref{a_div_b_then_a_leq_b}, contradicting the fact that $t$ was the least element of $T$. Therefore, our initial assumption that $T$ was nonempty is false and therefore $T$ must be empty, therefore showing that every integer has a unique prime factorization.
\end{proof} 
